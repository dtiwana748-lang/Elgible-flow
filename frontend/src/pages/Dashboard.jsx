...
function PlannerReportCard({ group, year, selectedBatch = "ALL", collapsed = false, onToggleExpanded, canEdit, canRequest, requests = [], onRequest, onReload }) {
  ...
  const downloadPdfReport = () => {
    ...
    const seriesBars = (key) => metricSeries.map(series => `<div class="bar-set"><div class="bar ${series.className}" style="height:${Math.max(8, Math.min(100, Math.round((Number(series.data[key] || 0) / Math.max(1, targetAllotted, totalSelections, s.floated || 0)) * 100)))}%"><span>${escapeHtml(series.data[key] || 0)}</span></div></div>`).join("");
    const performanceChart = `<section class="chart-card performance-card"><h2>PERFORMANCE OVERVIEW</h2><div class="legend">${metricSeries.map(series => `<span><i class="${series.className}"></i>${escapeHtml(series.label)}</span>`).join("")}</div><div class="bar-chart">${performanceMetrics.map(([label, key]) => `<div class="chart-column"><div class="bar-stack">${seriesBars(key)}</div><strong>${escapeHtml(label)}</strong></div>`).join("")}</div><p>Performance is calculated from the selected officer's linked planner rows.</p></section>`;
    const closureChart = `<section class="chart-card closure-card"><h2>CLOSURE PERFORMANCE (%)</h2><div class="closure-grid">${closureMetrics.map(([label, key], index) => `<article class="closure-metric tone-${index + 1}"><b>${escapeHtml(metricSeries[0].data[key] || 0)}%</b><strong>${escapeHtml(label)}</strong><small>Overall since inception</small></article>`).join("")}</div></section>`;
    const bottomPanels = `<div class="bottom-grid"><section class="info-card sales-card"><h2>SALES VS CORE FOCUS</h2><div class="focus-metric sales"><b>${salesCount}</b><strong>Sales Companies</strong><p>${salesPct}% of the current company mix.</p></div><div class="focus-metric core"><b>${coreCount}</b><strong>Core Companies</strong><p>${corePct}% of the current company mix.</p></div></section><section class="info-card highlights-card"><h2>KEY HIGHLIGHTS</h2><ul><li><span>✓</span><p><b>${escapeHtml(salesInsight)}</b>${salesCount} sales companies and ${coreCount} core companies.</p></li><li><span>✓</span><p><b>${s.closed || 0} companies closed</b> out of ${s.floated || 0} floated companies.</p></li><li><span>✓</span><p><b>${closureNilPct}% nil-selection closure</b> needs attention for better conversion.</p></li><li><span>✓</span><p><b>${totalSelections} total selections</b> with focus on consistent quality placements.</p></li></ul></section><section class="info-card action-card"><h2>KEY TAKEAWAYS &amp; ACTION PLAN</h2><ul><li>Improve closure with selection through stronger student readiness.</li><li>Reduce nil-selection closures through better pre-qualification.</li><li>Continue leveraging Sales-domain strength.</li><li>Build stronger Core-domain opportunities.</li><li>Maintain consistent follow-ups to improve closure performance.</li></ul></section></div>`;
    const footer = (page, total) => `<footer class="report-footer"><span>Placement Officer Report • ${escapeHtml(group.name)}</span><span>Generated on ${escapeHtml(reportDate)} • Page ${page} of ${total}</span></footer>`;
    const pages = [
      `<section class="pdf-page page-one"><header class="report-header"><div><div class="eyebrow">Placement Officer Report</div><h1>${escapeHtml(group.name)}<span>Placement Officer Report</span></h1><p>Batch: ${escapeHtml(selectedBatch === "ALL" ? "All Batches" : selectedBatch)} · Generated ${escapeHtml(reportDate)}</p></div><div class="header-chip">Data as on ${escapeHtml(reportDate)}</div></header><h2 class="section-title">PROFESSIONAL SUMMARY</h2><section class="summary">${summaryCards.map(card => `<article class="${summaryTone(card.key)}"><i></i><span>${escapeHtml(card.label)}</span><strong>${escapeHtml(card.value)}</strong><small>${escapeHtml(card.text)}</small></article>`).join("")}</section><main class="table-grid">${reportRows("Overall Performance Since Inception", overallRows)}${reportRows(batchTitle(batchOneName), batchRows(batchOne))}${reportRows(hasPastBatch ? batchTitle(batchTwoName) : "PAST BATCH", hasPastBatch ? batchRows(batchTwo) : [["Status", "No data available"]])}</main><p class="foot">This report is calculated directly from the linked company tracker.</p></section>`,
      `<section class="pdf-page page-two"><header class="continuation-header"><div><strong>Placement Officer Report - ${escapeHtml(group.name)}</strong><span>Performance Analysis</span></div><small>Data as on ${escapeHtml(reportDate)}</small></header><div class="charts">${performanceChart}${closureChart}</div>${bottomPanels}</section>`
    ];
    const totalPages = pages.length;
    printWindow.document.write(`<!doctype html><html><head><title>${escapeHtml(documentTitle)}</title><style>
      @page{size:A4 portrait;margin:0}*{box-sizing:border-box}html,body{margin:0;padding:0}body{font-family:Arial,sans-serif;color:#10233e;background:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact}.pdf-page{position:relative;width:210mm;min-height:297mm;margin:0 auto;padding:10mm 11mm 13mm;background:#fff;break-after:page;page-break-after:always}.pdf-page:last-child{break-after:auto;page-break-after:auto}.report-header{display:flex;justify-content:space-between;gap:18px;padding:15px 18px;background:#071b3a;color:#fff;border-radius:8px}.eyebrow{color:#ffb1bc;font-size:9px;font-weight:900;letter-spacing:1.55px;text-transform:uppercase}.report-header h1{margin:5px 0;font-size:25px;line-height:1.05}.report-header h1 span{display:block;margin-top:4px;color:#cce5fa;font-size:14px;font-weight:650}.report-header p{margin:0;color:#d8eafb;font-size:10px}.header-chip{align-self:center;padding:8px 11px;border:1px solid #7eb8e7;border-radius:6px;font-size:10px;font-weight:800;white-space:nowrap}.continuation-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;padding:9px 12px;border-left:5px solid #0a5ea8;background:#edf7ff;color:#071b3a}.continuation-header strong{display:block;font-size:13px}.continuation-header span{display:block;margin-top:2px;color:#0a5ea8;font-size:9px;font-weight:800;text-transform:uppercase;letter-spacing:.8px}.continuation-header small{color:#486078;font-size:9px;font-weight:800}.section-title{margin:11px 0 0;padding:8px 10px;background:#d8f0ff;border:1px solid #8cccf1;border-bottom:0;border-radius:7px 7px 0 0;color:#073b78;text-align:center;font-size:12px;letter-spacing:1.25px}.summary{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:7px;padding:8px;border:1px solid #8cccf1;border-radius:0 0 7px 7px}.summary article{position:relative;min-height:99px;padding:9px 8px 8px;border:1px solid #b9d9ef;border-left:4px solid var(--tone);border-radius:7px;background:#fff}.summary article i{display:block;width:22px;height:5px;margin-bottom:7px;border-radius:99px;background:var(--tone)}.summary span{display:block;color:#36516d;font-size:8.5px;font-weight:900;text-transform:uppercase;line-height:1.2}.summary strong{display:block;margin:6px 0 4px;color:var(--tone);font-size:24px;line-height:1}.summary small{display:block;color:#516981;font-size:8.2px;line-height:1.25}.tone-blue{--tone:#0a5ea8}.tone-green{--tone:#07937b}.tone-purple{--tone:#7048b8}.tone-orange{--tone:#e58217}.tone-red{--tone:#d12f5a}.table-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-top:11px}.metric-card,.chart-card,.info-card{overflow:hidden;border:1px solid #83c9ec;border-radius:7px;background:#fff;break-inside:avoid;page-break-inside:avoid}.metric-card h2,.chart-card h2,.info-card h2{margin:0;padding:8px 7px;background:#d8f0ff;color:#073b78;text-align:center;font-size:10.4px;line-height:1.15;letter-spacing:.55px}.metric-card table{width:100%;border-collapse:collapse;table-layout:fixed}.metric-card th,.metric-card td{padding:6px 7px;border-bottom:1px solid #d7e8f2;font-size:8.8px;line-height:1.18}.metric-card th{width:72%;color:#314a64;text-align:left;font-weight:700}.metric-card td{width:28%;color:#071b3a;text-align:right;font-weight:900;white-space:nowrap}.metric-card tr:last-child>*{border-bottom:0}.foot{margin:8px 2px 0;color:#536b82;font-size:8.5px}.charts{display:grid;grid-template-columns:1.2fr .9fr;gap:9px}.chart-card{min-height:111mm}.legend{display:flex;flex-wrap:wrap;justify-content:center;gap:9px;padding:8px 9px 0;color:#314a64;font-size:8.6px;font-weight:800}.legend span{display:flex;align-items:center;gap:4px}.legend i{width:10px;height:10px;border-radius:2px;box-shadow:inset 0 0 0 1px rgba(0,0,0,.08)}.dark{background:#071b3a}.mid{background:#d63847}.light{background:#0a5ea8}.bar-chart{height:84mm;display:flex;align-items:stretch;justify-content:space-around;gap:5px;margin:7px 11px 0;padding:0 5px 5px;border-bottom:1px solid #9dbbd0}.chart-column{display:flex;flex:1;min-width:0;flex-direction:column;justify-content:flex-end;align-items:center;gap:6px}.bar-stack{height:65mm;width:100%;display:flex;align-items:flex-end;justify-content:center;gap:4px}.bar-set{height:100%;display:flex;align-items:flex-end}.bar{position:relative;width:14px;min-height:9px;border-radius:3px 3px 0 0}.bar span{position:absolute;bottom:calc(100% + 3px);left:50%;transform:translateX(-50%);font-size:7.5px;font-weight:900;color:#10233e}.chart-column>strong{min-height:24px;color:#334d67;font-size:8px;line-height:1.15;text-align:center}.chart-card>p{margin:5px 10px 7px;color:#5c7187;font-size:8.4px;text-align:center}.closure-grid{display:grid;grid-template-columns:1fr;gap:11px;padding:17px 12px}.closure-metric{display:grid;grid-template-columns:76px 1fr;align-items:center;gap:10px;padding:9px;border:1px solid #b6d9ee;border-left:5px solid var(--tone);border-radius:8px;background:#fff}.closure-metric b{display:grid;place-items:center;width:72px;min-height:47px;border-radius:8px;background:var(--tone);color:#fff;font-size:22px;line-height:1}.closure-metric strong{display:block;color:#203b57;font-size:9.5px;line-height:1.2}.closure-metric small{display:block;margin-top:4px;color:#6a7d8f;font-size:7.8px}.tone-1{--tone:#071b3a}.tone-2{--tone:#07937b}.tone-3{--tone:#d63847}.bottom-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px;margin-top:9px;break-inside:avoid;page-break-inside:avoid}.info-card{min-height:112mm}.info-card h2{background:#d8f0ff}.sales-card{display:grid;grid-template-rows:auto 1fr 1fr}.focus-metric{display:grid;align-content:center;gap:5px;margin:10px;padding:10px;border-radius:8px;background:#fff8ef;border:1px solid #f0c087;border-left:5px solid #e58217}.focus-metric.core{background:#eff8ff;border-color:#9dccec;border-left-color:#0a5ea8}.focus-metric b{color:#071b3a;font-size:31px;line-height:1}.focus-metric strong{color:#173857;font-size:10px;text-transform:uppercase}.focus-metric p{margin:0;color:#5c7187;font-size:8.5px;line-height:1.35}.highlights-card ul{display:grid;gap:7px;margin:0;padding:10px;list-style:none}.highlights-card li{display:grid;grid-template-columns:17px 1fr;gap:6px;padding:7px;border-radius:7px;background:#f4fbff;border:1px solid #cde8f7}.highlights-card span{display:grid;place-items:center;width:17px;height:17px;border-radius:50%;background:#07937b;color:#fff;font-size:10px;font-weight:900}.highlights-card p,.action-card li{margin:0;color:#334d67;font-size:8.8px;line-height:1.33}.highlights-card b{display:block;color:#071b3a;font-size:9.2px}.action-card{background:#fff7f9;border-color:#f0b7c4}.action-card h2{background:#d63847;color:#fff}.action-card ul{display:grid;gap:8px;margin:0;padding:12px 13px 12px 24px}.action-card li{padding-left:2px}.report-footer{position:absolute;left:11mm;right:11mm;bottom:5mm;display:flex;justify-content:space-between;border-top:1px solid #c8d8e5;padding-top:4px;color:#587087;font-size:8px;font-weight:700}@media print{body{background:#fff}.pdf-page{margin:0;box-shadow:none}.metric-card,.chart-card,.info-card,.summary,.table-grid,.charts,.bottom-grid{break-inside:avoid;page-break-inside:avoid}}
    </style></head><body>${pages.map((page, index) => page.replace(/<\/section>$/, `${footer(index + 1, totalPages)}</section>`)).join("")}</body></html>`);
