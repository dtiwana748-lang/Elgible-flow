...
async function run() {
  ...
  console.log('Matched by rollNo:', matchedByRoll.length, matchedByRoll.map(s => `${s.name} (${s.rollNo})`));
  console.log('Matched by enrollmentNo:', matchedByEnrollment.length, matchedByEnrollment.map(s => `${s.name} (${s.enrollmentNo})`));
  console.log('Matched by registrationNo:', matchedByReg.length, matchedByReg.map(s => `${s.name} (${s.registrationNo})`));
  console.log('Matched by grNo:', matchedByGr.length, matchedByGr.map(s => `${s.name} (${s.grNo})`));
  console.log('Matched by studentId:', matchedByStudentId.length, matchedByStudentId.map(s => `${s.name} (${s.studentId})`));
  console.log('Matched by universityId:', matchedByUniId.length, matchedByUniId.map(s => `${s.name} (${s.universityId})`));
